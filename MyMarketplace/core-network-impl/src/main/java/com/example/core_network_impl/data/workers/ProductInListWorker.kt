...
class ProductInListWorker(context: Context, workerParams: WorkerParameters) : Worker(
    context,
    workerParams
) {
    ...
    override fun doWork(): Result {
        return try {
            workerRepository.getProductsInList()
            Result.success()
        } catch (e: Exception) {
            if (runAttemptCount < COUNT_OF_WORKER_ENTRIES) {
                Result.retry()
            } else {
                Result.failure()
            }
    ...
    companion object {
        private const val COUNT_OF_WORKER_ENTRIES = 5
    }
}